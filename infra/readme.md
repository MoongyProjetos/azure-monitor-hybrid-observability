**script PowerShell pronto pra rodar no Azure Cloud Shell (PowerShell)** que cria, em um **Resource Group**, os principais artefatos de infra do workshop:
...
* Action Group (e-mail)
* Alertas (métrica)
* Data Collection Rules (DCR) + Associations (para uma VM opcional)
* AKS + ACR (opcional, porque é caro/demorado)
* ACI (opcional)
...
> Cole tudo no Cloud Shell (PowerShell).
> Ajuste **prefixo**, **região**, **email** e **senha do SQL**.
...
## O que esse script te entrega (na prática)

* Um “ambiente de laboratório” completo, coerente com as sessões 1–10
* Base pronta pra:
  ...
  * App Insights
  * Dados/Storage
  * Containers (se você ligar os flags)
  * Híbrido (Arc via passo manual)
